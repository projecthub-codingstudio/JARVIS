...
## Privacy: External Network Calls

JARVIS is designed as a **local-first, offline-capable** system. All LLM inference, embedding, STT/TTS, and document processing run on-device. However, certain **user-triggered features** require network access:

| Feature | Destination | When |
|---------|-------------|------|
| Weather lookup | `wttr.in`, `nominatim.openstreetmap.org` | User asks "날씨 알려줘" |
| Web search | `api.duckduckgo.com` | User asks web search questions |
| Model download (first-run) | `huggingface.co` | When a new model alias is used for the first time |

**What does NOT make network calls:**
- All RAG queries (retrieval, LLM generation, citation)
- Document indexing, chunking, embedding
- Session Query Learning (capture, detection, injection)
- File browsing, viewing
- Voice STT/TTS (fully local after model download)
- No telemetry, no analytics, no crash reporting

At startup, `HF_HUB_OFFLINE=1` is set by default to prevent silent HuggingFace network checks. Override with `HF_HUB_OFFLINE=0` when downloading new models.
...
JARVIS는 **로컬 우선, 오프라인 가능** 시스템입니다. 모든 LLM 추론, 임베딩, STT/TTS, 문서 처리가 디바이스에서 실행됩니다. 단, 일부 **사용자가 명시적으로 호출하는 기능**은 네트워크 접근이 필요합니다:

| 기능 | 대상 | 시점 |
|------|------|------|
| 날씨 조회 | `wttr.in`, `nominatim.openstreetmap.org` | "날씨 알려줘" 질문 시 |
| 웹 검색 | `api.duckduckgo.com` | 웹 검색 질문 시 |
| 모델 다운로드 (최초 1회) | `huggingface.co` | 새 모델 별칭 처음 사용 시 |

**네트워크를 사용하지 않는 기능:** RAG 쿼리, 문서 인덱싱/임베딩, Session Query Learning, 파일 탐색/뷰어, 음성 STT/TTS (다운로드 후), 텔레메트리/분석 일체 없음.

시작 시 `HF_HUB_OFFLINE=1`이 기본 설정되어 HuggingFace 자동 네트워크 체크가 차단됩니다. 새 모델 다운로드가 필요하면 `HF_HUB_OFFLINE=0`으로 오버라이드.

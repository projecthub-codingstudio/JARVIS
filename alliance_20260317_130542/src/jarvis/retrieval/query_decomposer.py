...
class QueryDecomposer:
    ...
    def _normalize_query(self, query: str) -> str:
        ...
        if _should_apply_filename_rewrite(query, rewrite.candidates):
            return rewrite.rewritten_query
        return query


def _should_apply_filename_rewrite(query: str, candidates: tuple[object, ...]) -> bool:
    if not _FILENAME_REWRITE_HINT_RE.search(query):
        return False

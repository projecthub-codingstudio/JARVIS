...
JARVIS runs entirely on your MacBook Pro with **zero cloud dependencies**. It provides document-grounded Q&A with citations, hybrid query planning, and real-time knowledge base indexing — all processed locally on Apple Silicon.
...
| :kr: | **Korean-first** | Native Korean morphological analysis (Kiwi), heuristic + lightweight bilingual query planning, optimized for Korean documents. |

## Key Features

- **Personal RAG** — Document-grounded Q&A over PDF, DOCX, XLSX, HWP, Markdown, and code files
- **Citation-backed Answers** — Factual answers require retrieved source evidence with citations
- **Hybrid Query Planner** — Heuristic-first planning with lightweight Korean-English keyword enrichment
...
|-----------|-----------|
| LLM | qwen3.5:9b (default) / MLX primary + Ollama fallback |
| Query Planner | Heuristic baseline + lightweight bilingual enrichment |
...
JARVIS는 MacBook Pro 위에서 **완전히 로컬로** 동작합니다. 클라우드 의존 없이 문서 기반 Q&A(출처 표시 포함), 하이브리드 쿼리 플래닝, 실시간 지식 베이스 인덱싱을 제공합니다.
...
| :kr: | **한국어 우선** | Kiwi 형태소 분석, 휴리스틱 + 경량 한영 쿼리 플래닝, 한국어 문서 최적화. |

### 주요 기능

- **개인 RAG** — PDF, DOCX, XLSX, HWP, Markdown, 코드 파일 기반 문서 Q&A
- **출처 기반 답변** — 사실 답변은 검색된 소스 증거와 인용이 있을 때만 생성
